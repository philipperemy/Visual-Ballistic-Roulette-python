...
def next_batch(debug=True):
    ...
    cutoff_diff_time = abs_cutoff_time - abs_times[-1]
    number_cutoff = get_number(abs_cutoff_time, initial_number, rev_time)
    if debug:
        print('-' * 80)
        print('ABSOLUTE TIMES   =', abs_times)
        print('LAP TIMES        =', diff_times)
        print('CUTOFF DIFF TIME =', cutoff_diff_time)
        print('NUMBERS          =', numbers)
        print('NUMBER CUTOFF    =', number_cutoff)
    # plot_a(diff_times)
    # sleep(0.1)
    return {'abs_times': abs_times,
            'lap_times': diff_times,
            'cutoff_diff_time': cutoff_diff_time,
            'numbers': np.array(numbers),
